...
// MongoDB connection
//new db:logsitic db
mongoose.connect('mongodb://localhost:27017/logisticdb', { useNewUrlParser: true, useUnifiedTopology: true })
...
app.post('/logistics', async (req, res) => {
  ...
  if (!orderid || !phone || !email || !destination || !shipping) {
    return res.status(400).json({ message: 'orderid, phone, email, destination and shipping are required' });
  }
...
app.put('/logistics/:id', async (req, res) => {
  ...
  try {
    const updatedLog = await Logistic.findByIdAndUpdate(id, { orderid, phone, email, destination , shipping }, { new: true });
    if (!updatedLog) {
